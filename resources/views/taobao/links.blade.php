...
<div class="m-2 d-flex justify-content-center">
	<div class="d-flex flex-column">
		@forelse($prices->where('product_id', null) as $price)
		...
		@endforelse
		@foreach($prices->where('product_id') as $price)
			<div class="product-card">
				<div class="product-card__left">
					<a href="#" target="_blank">
						<img class="product-thumbnail" src="{{ $price->product->image->url ?? asset('storage/icons/ImagePlaceholder.svg') }}" alt="">
					</a>
				</div>
				<div class="product-card__right">
					<div><a href="{{$price->taobao_product->url}}" target="_blank" class="product-card__headline">{{ $price->description }}</a></div>
					...
					<div class="">
						<button type="button" class="mdc-button mdc-button--outlined mdc-button--error ignore-button mr-2">忽略</button>
						<button type="button" class="mdc-button mdc-button--unelevated confirm-button">修改</button>
					</div>
				</div>
			</div>
		@endforeach
